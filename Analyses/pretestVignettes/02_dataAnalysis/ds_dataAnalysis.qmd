...
- Cluster 3 (n=47, 68.1%) = likely baseline/human-like majority
  - Lowest risk score (0.234)
  - Highest visual accuracy (0.961)
  - Moderate RT (~21.3s) and duration (~722s)
  - Low paste, moderate mouse, low defocus
- Cluster 2 (n=5, 7.2%) = high-risk slow + low-accuracy pattern
  - Highest risk score (2.2)
  - Very low visual accuracy (0.30)
  - Long visual RT (~39.8s) + long duration (~1450s)
  - Very low paste/defocus
  - More consistent with low-quality / struggling / disengaged responders than fast bots
- Cluster 1 (n=1, 1.4%) = single extreme case
  - Risk score 2.0, high paste (9), high defocus (9), lower mouse (272)
  - Since n=1, treat as individual anomaly, not a stable subgroup
- Cluster 4 (n=6, 8.7%) = medium-risk, slow with elevated defocus
  - Risk 1.0, good-but-not-top accuracy (0.861)
  - Long RT (~37.3s), long duration (~1514s)
  - Defocus relatively high (4.33)
  - Looks like distracted / interrupted participants
- Cluster 5 (n=10, 14.5%) = active but low-risk variant
  - Low risk (0.7), good accuracy (0.883)
  - Very high mouse activity (~1194) and very long RT (~60.9s)
  - Could reflect careful/slow navigation, not necessarily suspicious

```{r}
tmp_pids <- suspected_bot_table$PROLIFIC_PID[suspected_bot_table$bot_cluster == 1]
tmp_pids

tmp_pids <- suspected_bot_table$PROLIFIC_PID[suspected_bot_table$bot_cluster == 2]
tmp_pids

tmp <- rowSums(dat_ques[dat_ques$PROLIFIC_PID %in% tmp_pids, str_subset(string = colnames(dat_ques), pattern = "^visual.*_correct$")])
barplot(table(tmp), main = "Cluster 2")
