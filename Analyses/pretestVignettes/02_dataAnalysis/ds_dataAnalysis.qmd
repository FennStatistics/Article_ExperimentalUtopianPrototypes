...
# 1) drop columns with any NA
vars_with_na <- colnames(cluster_features)[colSums(is.na(cluster_features)) > 0]
vars_with_na

cluster_features_noNAvars <- cluster_features %>%
  dplyr::select(-dplyr::all_of(vars_with_na))

# 2) also drop zero-variance columns, which would become NaN after scale()
vars_zero_var <- names(cluster_features_noNAvars)[
  vapply(cluster_features_noNAvars, function(x) stats::sd(x, na.rm = TRUE) == 0, logical(1))
]
vars_zero_var

cluster_features_ready <- cluster_features_noNAvars %>%
  dplyr::select(-dplyr::all_of(vars_zero_var))

# 3) scale and convert back to data frame
cluster_features_scaled_noNAvars <- cluster_features_ready %>%
  scale(center = TRUE, scale = TRUE) %>%
  as.data.frame()

# final checks
#colSums(is.na(cluster_features_scaled_noNAvars))
#sapply(cluster_features_scaled_noNAvars, sd)

# scale and convert back to data frame
cluster_features_scaled_noNAvars <- cluster_features_noNAvars %>%
  scale(center = TRUE, scale = TRUE) %>%
  as.data.frame()
